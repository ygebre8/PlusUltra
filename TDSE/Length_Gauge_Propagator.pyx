...
def Crank_Nicolson_Time_Propagator(input_par, psi_inital):
    ...
    if elliptical_pulse == False:
        ...
        if np.dot(total_polarization, np.array([1,0,0])) != 0:   
            ...
            Int_Ham_X = eval("Int." + "X_Matrix_" + input_par["block_type"] + "(input_par)")
        ...
        if np.dot(total_polarization, np.array([0,1,0])) != 0 : 
            ...
            Int_Ham_Y = Int_Ham_Temp_Right.duplicate()
            Int_Ham_Y.axpy(-1.0, Int_Ham_Temp_Left,structure=PETSc.Mat.Structure.DIFFERENT_NONZERO_PATTERN)
            Int_Ham_Y.scale(-0.25 * input_par["time_spacing"])
